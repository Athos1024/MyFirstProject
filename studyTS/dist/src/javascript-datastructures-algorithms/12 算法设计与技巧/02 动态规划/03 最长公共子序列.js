function lcs(wordX, wordY) {
    var f = [];
    //初始化 f
    for (var j = 0; j <= wordX.length; j++) {
        f[j] = [];
        f[j][0] = { value: 0, prev: null, x: j };
    }
    for (var i = 1; i <= wordY.length; i++) {
        f[0][i] = { value: 0, prev: null, x: 0 };
    }
    //循环计算
    for (var i = 1; i <= wordX.length; i++) {
        for (var j = 1; j <= wordY.length; j++) {
...
//c[ 2 3 ] =
// [A ,A ,C]
//# sourceMappingURL=03%20%E6%9C%80%E9%95%BF%E5%85%AC%E5%85%B1%E5%AD%90%E5%BA%8F%E5%88%97.js.map